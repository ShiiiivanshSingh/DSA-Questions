...
public:
    vector<int> minCost(vector<int>& nums, vector<vector<int>>& queries) {
        ...
        // back
        for (int i = n - 1; i > 0; i--) {
            if (closest[i] == i - 1) backward[i - 1] = backward[i] + 1;
             else  backward[i - 1] = backward[i] + (nums[i] - nums[i - 1]);
        ...
        for (auto &q : queries) {
            ...
            if (l < r) ans.push_back(forward[r] - forward[l]);
              else ans.push_back(backward[r] - backward[l]);
        }
