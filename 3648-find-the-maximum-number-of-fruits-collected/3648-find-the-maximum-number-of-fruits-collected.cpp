...
public:
    ...
    int childDos(vector<vector<int>>& fruits, int i, int j) {
        ...
        if (i == n - 1 && j == n - 1)
            return 0;
        // child2 = fruits[i][j] + max({childDos(fruits, i + 1, j),
        ...
        if (best == INT_MIN)
            return dpDos[i][j] = INT_MIN;

        return dpDos[i][j] = fruits[i][j] + best;
    ...
    int childTres(vector<vector<int>>& fruits, int i, int j) {
        ...
        if (i == n - 1 && j == n - 1)
            return 0;
        // child3 = fruits[i][j] + max({childTres(fruits, i - 1, j + 1),
        ...
        if (best == INT_MIN)
            return dpTres[i][j] = INT_MIN;

        return dpTres[i][j] = fruits[i][j] + best;
    ...
    int maxCollectedFruits(vector<vector<int>>& fruits) {
        ...
        return childUno(fruits) + childDos(fruits, 0, n - 1) +
               childTres(fruits, n - 1, 0);
    }
    };
